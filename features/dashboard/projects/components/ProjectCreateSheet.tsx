...
export function ProjectCreateSheet({ open, onOpenChange }: ProjectCreateSheetProps) {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    const payload = {
      ...
      services: data.services ? data.services.split(",").map((service) => service.trim()).filter(Boolean) : [],
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input placeholder="Branding, Web Design, Strategy" {...field} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {form.watch("thumbnailUrl") && (
                        <span className="text-sm text-muted-foreground">Thumbnail uploaded</span>
                      )}
                    </div>
                  </div>
